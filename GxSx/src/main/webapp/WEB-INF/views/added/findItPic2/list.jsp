...
<tbody id="FindItPic">
<c:if test="${empty findItPic2}">
	<tr align="center" noshade>
	   <td colspan="5">데이터가 하나도 없음</td>
	</tr>
</c:if>
<c:forEach items="${findItPic2}" var="FindItPic">
	<TR align='center' noshade>
		<TD>${FindItPic.fino}</TD>
		<TD>${FindItPic.fisub}</TD>
	    <TD>${FindItPic.ficon}</TD>
		<TD>${FindItPic.fidate}</TD>
	   </TR> 
</c:forEach>
</tbody>      
</TABLE>
...
<nav aria-label="...">
  ...
  <ul id="paging" class="pagination" style="display:inline-flex">
    <li class="page-item disabled" onclick="selectFindItPicPage(${listpage.startPage-1})">
      <span class="page-link" >Previous</span>
    </li>
    <c:forEach var="index" begin="${listpage.startPage}" end="${listpage.endPage}" step="1">
    	<c:choose>
    		<c:when test="${index==listpage.currentPage}">
			    <li id="pageNum${index}" class="page-item active" onclick="selectFindItPicPage(${index})">
    		...
    		<c:otherwise>
		   		<li id="pageNum${index}" class="page-item" onclick="selectFindItPicPage(${index})">
		   			<span class="page-link">
		   				${index}
		   			</span>
		   		</li>
    ...
    <li class="page-item disabled" onclick="selectFindItPicPage(${listpage.endPage+1})">
...
<input id="pageCount" type="hidden" value="${listpage.pageCount}">
	...
	<!— 자바스크립트  —>
	<script src="../js/FindItPic.js"></script>
